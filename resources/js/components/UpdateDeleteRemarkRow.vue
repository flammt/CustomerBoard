<template>
    <div id="container" class="form-row">
        ...
        <h5 v-else style="align-self: start">{{model.type.name}}</h5>

        <h5 v-if="edit" style="width:100%">
            <input style="width:95%" type="text" :value="inputRemark" @input="remarkInput($event.target.value)">
        </h5>
        <h5 v-else style="width:100%;">{{model.text}}</h5>
    ...
    export default {
        ...
        methods: {
            ...
            ok () {
                ...
                this.$emit('createUpdateConfirmed', this.model.id, this.selectedType, this.inputRemark);
            },
            switchEdit () {
                this.edit = !this.edit;
            },
